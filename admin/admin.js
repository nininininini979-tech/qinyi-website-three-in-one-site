(function () {
  ...
  async function loadImportant() {
    ...
    try {
      ...
      setHtml("importantFormBody", `<div class="detail-section"><h3>待企业确认的公开事实</h3><p class="cell-subtitle">以下资料在访客页面按现有版本展示，同时在这里保留“待补充”标记；确认后由 Agent 生成内容草稿并进入人工发布流程。</p></div><div class="form-grid">${inputField("公司中英文法定名称与品牌关系", "legalBrandFacts", fields.legalBrandFacts || "待补充", { multiline: true, wide: true })}${inputField("联系人完整中文姓名", "contactIdentity", fields.contactIdentity || "待补充")}${inputField("工厂地址（上园村/上元村）", "addressConfirmation", fields.addressConfirmation || "待补充")}${inputField("成立年份、厂房面积与团队人数", "factoryProfileConfirmation", fields.factoryProfileConfirmation || "待补充", { multiline: true, wide: true })}${inputField("MOQ、响应时间与现有刀模尺寸", "commercialFacts", fields.commercialFacts || "待补充", { multiline: true, wide: true })}${inputField("设备、认证与检测记录的现行范围", "certificationFacts", fields.certificationFacts || "待补充", { multiline: true, wide: true })}${inputField("业务邮箱", "contactEmail", fields.contactEmail, { type: "email" })}${inputField("联系电话", "contactPhone", fields.contactPhone)}${inputField("当前生产周期", "leadTime", fields.leadTime)}${inputField("起订量说明", "moq", fields.moq)}${inputField("近期假期与停工安排", "holidayNotice", fields.holidayNotice, { multiline: true, wide: true })}${inputField("临时业务说明", "businessNotice", fields.businessNotice, { multiline: true, wide: true })}${inputField("禁止承诺事项", "restrictedCommitments", Array.isArray(fields.restrictedCommitments) ? fields.restrictedCommitments.join("\n") : fields.restrictedCommitments, { multiline: true, wide: true })}</div><p class="cell-subtitle">当前版本：${Ops.escapeHtml(data.revision || "未发布")} · 最近更新：${Ops.formatTime(data.updatedAt, true)}</p>`);
